...
class NodeRtmpSession {
  ...
  onConnect(invokeMessage) {
    ...
    context.nodeEvent.emit('preConnect', this.id, this.ip, invokeMessage.cmdObj);
    if (!this.isStarting) {
      return;
    }
  ...
  onPublish(invokeMessage) {
    ...
    context.nodeEvent.emit('prePublish', this.id, this.ip, this.publishStreamPath, this.publishArgs);
    if (!this.isStarting) {
      return;
    }
